...
#vmess
     ...
     {
     "listen": "/run/xray/vmess_ws_worryfree.sock",
     "protocol": "vmess",
      "settings": {
            "clients": [
               {
...
#vmessWorryfree
     ...
     {
     "listen": "/run/xray/vmess_ws_kuota_habis.sock",
     "protocol": "vmess",
      "settings": {
            "clients": [
               {
...
sed -i '$ i{' /etc/nginx/conf.d/xray.conf
sed -i '$ iproxy_redirect off;' /etc/nginx/conf.d/xray.conf
sed -i '$ iproxy_pass http://unix:/run/xray/vmess_ws_worryfree.sock;' /etc/nginx/conf.d/xray.conf
sed -i '$ iproxy_http_version 1.1;' /etc/nginx/conf.d/xray.conf
sed -i '$ iproxy_set_header X-Real-IP \$remote_addr;' /etc/nginx/conf.d/xray.conf
sed -i '$ iproxy_set_header X-Forwarded-For \$proxy_add_x_forwarded_for;' /etc/nginx/conf.d/xray.conf
sed -i '$ iproxy_set_header Upgrade \$http_upgrade;' /etc/nginx/conf.d/xray.conf
...
sed -i '$ i{' /etc/nginx/conf.d/xray.conf
sed -i '$ iproxy_redirect off;' /etc/nginx/conf.d/xray.conf
sed -i '$ iproxy_pass http://unix:/run/xray/vmess_ws_kuota_habis.sock;' /etc/nginx/conf.d/xray.conf
sed -i '$ iproxy_http_version 1.1;' /etc/nginx/conf.d/xray.conf
sed -i '$ iproxy_set_header X-Real-IP \$remote_addr;' /etc/nginx/conf.d/xray.conf
sed -i '$ iproxy_set_header X-Forwarded-For \$proxy_add_x_forwarded_for;' /etc/nginx/conf.d/xray.conf
sed -i '$ iproxy_set_header Upgrade \$http_upgrade;' /etc/nginx/conf.d/xray.conf
...
sleep 2
echo -e "[ ${green}INFO$NC ] Installing Domain.."
...
/root/.acme.sh/acme.sh --issue -d $domain --standalone -k ec-256 -ak ec-256
echo -e "${green}Membuat Sertifikat...${NC}"
...
touch /etc/xray/shadowsocks-client.conf
